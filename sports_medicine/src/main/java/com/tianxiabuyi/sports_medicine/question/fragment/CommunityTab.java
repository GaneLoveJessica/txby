 ...
 * 问答列表页面
 */
public class CommunityTab extends BaseRefreshFragment<Question> {
    private long categoryId;
    private String searchContent;
    private boolean isSearch;

    public static CommunityTab newInstance(long categoryId, boolean isSearch) {
        CommunityTab communicateTab = new CommunityTab();
    ...
    protected List<Question> getCache() {
        if (!isSearch) {
            return CacheUtil.getQuestion(getActivity(), CacheUtil.KEY_COMMUNITY + categoryId);
        }
        return null;
    }
    ...
    protected List<Question> getResult(HttpResult httpresult, boolean isRefresh) {
        final List<Question> questions = httpresult.getListResult("quests", new TypeToken<List<Question>>() {
        });
        if (isRefresh && !isSearch) {
            CacheUtil.saveCache(getActivity(), CacheUtil.KEY_COMMUNITY + categoryId, questions);
        }
        return questions;
    }
